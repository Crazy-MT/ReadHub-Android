...
import com.maotong.readhub.ui.activity.ReadHubActivity;
...
public class ReadHubAdapter extends RecyclerView.Adapter<ReadHubAdapter.ReadHubViewHolder> {
    ...
    private static final int TIME_ITEM = 0;
    private static final int DETAIL_ITEM = 1;
    private CacheUtil mCacheUtil;

    public ReadHubAdapter(ArrayList<Datum> readHubHotItems, Context context) {
        this.readHubHotItems = readHubHotItems;
    ...
    public void onBindViewHolder(final ReadHubViewHolder holder, final int position) {
        final Datum readHubHotItem = readHubHotItems.get(holder.getAdapterPosition());
        if (DBUtils.getDB(mContext).isRead(Config.READHUB, readHubHotItem.getId(), 1))
        ...
        holder.mTvTime.setText(getTime(readHubHotItem.getCreatedAt()));
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DBUtils.getDB(mContext).insertHasRead(Config.READHUB, readHubHotItem.getId(), 1);
                holder.mTvTitle.setTextColor(Color.GRAY);

                //intent 传递数据太大，持久化数据解决
                mCacheUtil.put(Config.READHUB, new Gson().toJson(readHubHotItems));

                Intent intent = new Intent(mContext, ReadHubActivity.class);
                intent.putExtra(ReadHubActivity.POSITION,position);
                mContext.startActivity(intent);
            }
        });
        if (mSparseBooleanArray.get(readHubHotItem.getOrder())) {
        ...
        holder.btnDetail.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (holder.mTvDescription.getVisibility() == View.GONE) {
                    holder.btnDetail.setBackgroundResource(R.drawable.ic_expand_less_black_24px);
                    holder.mTvDescription.setVisibility(View.VISIBLE);
                    mSparseBooleanArray.put(readHubHotItem.getOrder(), true);
                } else {
                    holder.btnDetail.setBackgroundResource(R.drawable.ic_expand_more_black_24px);
                    holder.mTvDescription.setVisibility(View.GONE);
                    mSparseBooleanArray.put(readHubHotItem.getOrder(), false);
                }
            }
        });
